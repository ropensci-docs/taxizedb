...
- x:
  ...
  (character) Vector of taxon keys (ids) for the given database.
  required
...
- missing:
  ...
  (character) if no data found at the given rank and input key, should
  we get the next closest lower than that given in `rank`, or higher.
...
list of data.frame's for each input taxon key, where each data.frame has
fields: name, rank, id. When no results found, an empty data.frame
